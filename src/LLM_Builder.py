...
st.session_state["prompt_options"] = [prompt.get_name() for prompt in st.session_state["prompts"]]
...
with col2:
  ...
  with st.expander("Prompt Configuration"):
    ...
    prompt = next(filter(lambda x: x.get_name() == prompt_selection, st.session_state["prompts"]))
    ...
    if prompt:
        prompt_content = st.text_area('Prompt Text:', height=200, key='prompt_content', value=prompt['prompt'])
    else:
